...
void CheckArguments(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    printf("\033[0;31m"
           "One arguments needed, no more no less. Execution finished.\n");
    exit(1);
  }

  if (atol(argv[1]) > INT_MAX) {
    printf("\033[0;31m"
           "Too large of a number, does not fit in type `int`. Execution "
           "finished.\n");
    exit(1);
  }
  ...
  if ((atoi(argv[1]) < 1) || !isdigit(*argv[1])) {
    printf("\033[0;31m"
           "The argument must be a positive integer. Execution finished.\n");
    exit(1);
  }
}
...
// only defines a single package, another function is used to generate a list of
// packages
{
...
// function to simulate an insertion of RobotPackages in a ordered way (sorted
// by supplier)
...
// function to remove all the RobotPackages from the list at the end of the
// program
...
// function to remove all packages from a given stack when its MAX_CAPACITY is
// reached
void RemoveStack(struct Package *head, int idx) {
  ...
  if (head == NULL) {
    printf(
        "The stack is empty or pointer is wrongly provided (NULL pointer)\n");
    return;
  }
...
// function to simulate putting a generated Package to a corresponding stack
...
void CleanShoppingQueue() // on the template file this function has parameters
                          // into it - T
{
  int number = 0;
  if (queueFirst == NULL) {
  ...
  struct Shopping *next_robot; // here next is used as a temporal pointer in
                               // order to reassing current
...
void SimulationLoop(int EventNumbers) {
  ...
  int *numberThingsToBuy_Current_Served =
      malloc(sizeof(int)); // Allocate memory for the integer
  if (numberThingsToBuy_Current_Served != NULL) {
    *numberThingsToBuy_Current_Served =
        0; // Assign a value to the allocated memory
  }
  ...
  for (int i = 0; i < EventNumbers; i++) {
    ...
    if (BREAK_FLAG)
      break; // if an error occurred, exit for loop and clean the simulation
  }
